...
void EXTI0_1_IRQHandler(void)
{
  ...
  if(__HAL_GPIO_EXTI_GET_IT(Button0_Pin) !=RESET){
    ...
    // LL_GPIO_ResetOutputPin(GPIOA, DataOut_Pin);
  }
   if(__HAL_GPIO_EXTI_GET_IT(Button1_Pin) != RESET){
	  lcd_command(CLEAR);
	  lcd_putstring("World");
    // HAL_GPIO_TogglePin(GPIOB, LED4_Pin);
    // LL_GPIO_ReadInputPin(GPIOA,dataInput_Pin);
    if(LL_GPIO_IsInputPinSet(GPIOB,dataInput_Pin)) HAL_GPIO_TogglePin(GPIOB, LED4_Pin);
...
void getADCData(void){
  ...
  for(int bit_idx = 12 ; bit_idx > 0 ; bit_idx--){
      if(binary[bit_idx] == 1){
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_SET);  // High
        lcd_putstring("1");
      }else if( binary[bit_idx]== 0){
        HAL_GPIO_WritePin(GPIOA, GPIO_PIN_7, GPIO_PIN_RESET);  // Low
        lcd_putstring("0");
      }
      HAL_Delay(2500/txsn_rate);
    }
    lcd_command(CLEAR);

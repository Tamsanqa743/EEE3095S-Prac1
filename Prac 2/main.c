...
uint8_t counter = 1;  // counter for interrupt calls
...
int main(void)
{
  ...
  for (int i = 1; i <= 6; i++){
    uint16_t addr = 0x08*i;
    write_to_address(addr, patterns[i-1]);
  }
  ...
  while (1)
  {
    ...
    if(!(LL_GPIO_ReadInputPort(Button0_GPIO_Port) & Button0_Pin)){
      ...
      // GPIOA -> IDR &= 0x0000; // clear idr for pin 0
    }
...
void TIM16_IRQHandler(void)
{
  ...
  uint8_t data = read_from_address(0x08*counter); // read data from eeprom
  uint8_t status = 0;
  ...
  if (status){
    GPIOB -> ODR = data; // write to output data register
  }
  else{
    GPIOB -> ODR = 0x1;
  }

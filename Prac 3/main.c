...
int main(void)
{
  ...
  // poll adc 
  adc_val = pollADC();
  char polled_adc_value[16];
  sprintf(polled_adc_value, "Value: %u", adc_val);// convert to correct format to write to lcd
  // Write to lcd
  writeLCD(polled_adc_value);
...
void EXTI0_1_IRQHandler(void)
{
  ...
  delay(100); // delay to account for debouncing
  // Toggle LED frequency
  if (status){
    delay_t = 1000; // set delay to 1000
    status = 0;
  }
  else{
    delay_t = 500; // set delay to 500
    status = 1;
  }

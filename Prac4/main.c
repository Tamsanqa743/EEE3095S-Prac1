...
int main(void)
{
  ...
  for(int index = 0; index < NS;  index++){
	  Sin_LUT[index] = ((MAX_VALUE/2)*(sin(2*3.14*index/NS)+1));
  }

  for (int index = 0; index < NS; index++){
	  if (index < (NS/2)){
		  triangle_LUT[index] = (index*(MAX_VALUE+1))/(NS/2);
	  }
	  else{
		  triangle_LUT[index] = MAX_VALUE - ((index - NS/2)*(MAX_VALUE+1))/(NS/2);
	  }
  }

  for(int index = 0; index < NS; index++){
	  saw_LUT[index] = (index*(MAX_VALUE+1))/NS;
  }
...
void EXTI0_1_IRQHandler(void)
{
	...
	if(currentTime - prevButtonPress){
		...
		case 0:
			wave =(uint32_t*)Sin_LUT;
			lcd_command(CLEAR);
			lcd_putstring("Sine");
			break;
		case 1:
			wave =(uint32_t*)saw_LUT;
			lcd_command(CLEAR);
			lcd_putstring("Sawtooth");
			break;
		case 2:
			wave =(uint32_t*)triangle_LUT;
			lcd_command(CLEAR);
			lcd_putstring("Triangle");
			break;
		}

		HAL_DMA_Start_IT(&hdma_tim2_ch1, (uint32_t)wave, DestAddress,NS);
		__HAL_TIM_ENABLE_DMA(&htim2, TIM_DMA_CC1);

		prevButtonPress = currentTime;
	}
